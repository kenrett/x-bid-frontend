...
vi.mock("@features/auctions/hooks/useAuctionChannel", () => {
  const useAuctionChannel = vi.fn();
  return { useAuctionChannel };
});
vi.mock("@features/auctions/api/auctions", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@features/auctions/api/auctions")>();
  return {
    ...actual,
    getAuction: vi.fn(),
  };
});
vi.mock("@features/auctions/api/bids", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("@features/auctions/api/bids")>();
  return {
    ...actual,
    getBidHistory: vi.fn(),
    placeBid: vi.fn(),
...
} from "@features/auctions/hooks/useAuctionChannel";
import * as auctionsApi from "@features/auctions/api/auctions";
import * as bidsApi from "@features/auctions/api/bids";
